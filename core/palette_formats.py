...
Metadata (palette name, description, author) is embedded in formats
that support it and extracted on import.
...
@dataclass
class ImportResult:
    """Result of a palette import containing colors and optional metadata."""
    colors: list[ColorWeight] = field(default_factory=list)
...
class PaletteFormats:
    ...
    def export_palette(
        ...
        """Export palette based on file extension, embedding metadata where supported."""
        if not colors:
        ...
        export_method(path, colors, metadata)

    @staticmethod
    def import_palette(path: str) -> ImportResult:
        """Import palette based on file extension. Returns colors and metadata."""
        ext = os.path.splitext(path)[1].lower()
        ...
        import_methods = {
            ...
            # Optional imports with limitations
        ...
        result = import_method(path)

        # Normalise: old-style methods return a plain list, new ones return ImportResult
        if isinstance(result, ImportResult):
            return result
        return ImportResult(colors=result)
    ...
    @staticmethod
    def _palette_name(metadata: PaletteMetadata | None) -> str:
        """Get palette name from metadata or return default."""
        if metadata and metadata.is_named:
    ...
    @staticmethod
    def _export_aco(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        ...
        with open(path, 'wb') as f:
            ...
            for color, weight in colors:
                ...
                f.write(struct.pack('>HHHH',
                    color[0] * 257, color[1] * 257, color[2] * 257, 0))
    ...
    @staticmethod
    def _export_acb(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        """Export Adobe Color Book format."""
        name = PaletteFormats._palette_name(metadata)
        desc = metadata.description if metadata and metadata.has_notes else f"Generated by {name}"
        with open(path, 'wb') as f:
            f.write(b'8BCB')
            f.write(struct.pack('>H', 1))
            f.write(struct.pack('>H', 0))

            title = (name + "\0").encode('ascii', errors='replace')
            ...
            desc_bytes = (desc + "\0").encode('ascii', errors='replace')
    ...
    @staticmethod
    def _export_gpl(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        ...
        with open(path, 'w', newline='\n') as f:
            ...
            f.write(f"Name: {name}\n")
            f.write("Columns: 0\n")
            if metadata and metadata.has_notes:
    ...
    @staticmethod
    def _export_clr(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
    ...
    @staticmethod
    def _export_colors(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
    ...
    @staticmethod
    def _export_css(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        """Export CSS variables format."""
        name = PaletteFormats._palette_name(metadata)
        # newline='\n' disables Python's text-mode CRLF translation on Windows
        # so the file is byte-identical across platforms.
        with open(path, 'w', newline='\n') as f:
            f.write(f"/* {name} - CSS Variables */\n")
            if metadata and metadata.has_notes:
    ...
    @staticmethod
    def _export_json(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        ...
        # Embed metadata fields when available
        if metadata:
            if metadata.has_notes:
                palette_data["description"] = metadata.description
            if metadata.author:
                palette_data["author"] = metadata.author
            if metadata.created_at:
                palette_data["created_at"] = metadata.created_at
            if metadata.modified_at:
                palette_data["modified_at"] = metadata.modified_at

        for i, (color, weight) in enumerate(colors):
            palette_data["colors"].append({
                "name": f"Color {i+1}",
                "hex": ColorMath.rgb_to_hex(color),
                "rgb": {"r": color[0], "g": color[1], "b": color[2]},
                "weight": weight,
            })

        # ... palette_data construction unchanged
        # newline='\n' disables Python's text-mode CRLF translation on Windows
        # so the file is byte-identical across platforms.
        with open(path, 'w', newline='\n') as f:
    ...
    @staticmethod
    def _export_xml(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        """Export XML format with metadata."""
        name = PaletteFormats._palette_name(metadata)
        root = ET.Element('palette')
        root.set('name', name)
        root.set('version', '1.0')

        # Add metadata elements when available
        if metadata:
            if metadata.has_notes:
                ET.SubElement(root, 'description').text = metadata.description
            if metadata.author:
                ET.SubElement(root, 'author').text = metadata.author
        ...
        for i, (color, weight) in enumerate(colors):
            ...
            ET.SubElement(color_elem, 'hex').text = ColorMath.rgb_to_hex(color)
    ...
    @staticmethod
    def _export_hsv(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        """Export HSV format."""
        name = PaletteFormats._palette_name(metadata)
        # newline='\n' disables Python's text-mode CRLF translation on Windows
        # so the file is byte-identical across platforms.
    ...
    @staticmethod
    def _export_svg(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        """Export SVG palette."""
        name = PaletteFormats._palette_name(metadata)
        swatch_size = 60
        cols = 6
    ...
    @staticmethod
    def _export_hex(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
    ...
    @staticmethod
    def _export_hsl(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
        """Export HSL format."""
        name = PaletteFormats._palette_name(metadata)
        # newline='\n' disables Python's text-mode CRLF translation on Windows
        # so the file is byte-identical across platforms.
    ...
    @staticmethod
    def _export_txt(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
    ...
    @staticmethod
    def _export_procreate(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
    ...
    @staticmethod
    def _export_affinity(path: str, colors: list[tuple[tuple[int, int, int], int]], metadata: PaletteMetadata | None = None) -> None:
    ...
    @staticmethod
    def _import_gpl(path: str) -> ImportResult:
        """Import GIMP Palette format, extracting name from header."""
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if stripped.startswith('Name:'):
                    palette_name = stripped[5:].strip()
                    start_idx = i + 1
                elif stripped.startswith('GIMP') or stripped.startswith('Columns:'):
                    start_idx = i + 1
                elif stripped.startswith('#') and i > 0 and start_idx <= i:
                    # Comment lines before color data = description
            ...
            logger.error(f"Error importing GPL: {e}")

        meta = None
        if palette_name or description_lines:
            meta = PaletteMetadata(
                name=palette_name or "Untitled Palette",
                description="\n".join(description_lines),
            )
        return ImportResult(colors=colors, metadata=meta)
    ...
    @staticmethod
    def _import_json(path: str) -> ImportResult:
        """Import JSON format, extracting metadata when present."""
        colors: list[ColorWeight] = []
        meta: PaletteMetadata | None = None
        try:
            with open(path, 'r') as f:
                ...
                if isinstance(data, dict):
                    ...
                    if any(k in data for k in ("name", "description", "author")):
                        meta = PaletteMetadata(
    ...
    @staticmethod
    def _import_xml(path: str) -> ImportResult:
        """Import XML format, extracting metadata when present."""
        colors: list[ColorWeight] = []
        meta: PaletteMetadata | None = None
        try:
            tree = ET.parse(path)
            ...
            # Extract metadata from palette element
            palette_name = root.get('name', '')
            desc_elem = root.find('description')
            author_elem = root.find('author')
            if palette_name or desc_elem is not None or author_elem is not None:
                meta = PaletteMetadata(
                    name=palette_name or "Untitled Palette",
    ...
    def _import_colors(path: str) -> list[tuple[tuple[int, int, int], int]]:
        ...
        try:
            with open(path, 'r') as f:
                for line in f:
                    line = line.strip()
                    if line and not (line == '#' or line.startswith('# ')):
                        parts = line.split()
                        if len(parts) >= 5:
                            try:
                                hex_color = parts[0]
    ...
    @staticmethod
    def _import_affinity(path: str) -> ImportResult:
        """Import Affinity Designer palette format with metadata."""
        colors: list[ColorWeight] = []
        meta: PaletteMetadata | None = None
        try:
            with open(path, 'r') as f:
                ...
                # Extract metadata
                aff_name = data.get("name", "")
                aff_desc = data.get("description", "")
                aff_author = data.get("author", "")
                if aff_name or aff_desc or aff_author:
                    meta = PaletteMetadata(
                        name=aff_name or "Untitled Palette",
        ...
        try:
            with open(path, 'rb') as f:
                ...
                if colors:
                    ...
                    logger.info("This may be an incompatible Procreate version")
