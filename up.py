...
WHAT THIS REPOSITORY NEEDED.
    ...
    Correct on every version statement, and missing one package.

    tests/requirements-dev.txt   pytest-timeout>=2.4,<3.0
    the marker                   tests/test_brand_mirror.py

WHY THAT ONE PACKAGE MATTERS. tests/test_test_tooling_pins.py checks that
declared ranges match the fleet's; it cannot notice a package that is simply
absent, and this was absent. The previous round's script passed --timeout to
pytest here and exited 4 -- a USAGE error, not a test failure -- and the icon
builder got away with the same flag only because that container happened to
have the plugin installed. A declaration is what makes the flag safe.
