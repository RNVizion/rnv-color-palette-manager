...
WHAT CHANGES HERE.
    ...
    Three palette values, and a ramp step that loses its last consumer.

    ui/colors.py                   scrollbar_handle_hover, all three modes;
                                   GREY_60 and its provenance entry
    tests/test_constant_names.py   GREY_60 joins RETIRED, the survivor is
                                   asserted beside it, and the guard learns
                                   the fleet's delivery marker

GREY_60 (#606060) PAINTED EXACTLY ONE THING -- the dark scrollbar handle on
hover, here and in the transformer, where the key that held it is never
read. The gold takes that role, so the step had nothing left to paint and is
retired rather than left as an unused constant.

The pattern is this repository's own: GREY_F0 was collapsed onto GREY_EE on
2026-09-06 exactly this way.
