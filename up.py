...
Align rnv-color-palette-manager's light panel and dark input to the surfaces
the other apps use.
...
WHAT MOVES
  ...
  LIGHT panel_bg   #eeeeee -> #f5f5f5
  DARK  input_bg   #2a2a2a -> #1a1a1a

ONE OF THESE REMOVES AN EDGE. SAY SO PLAINLY.

This app was the only one drawing a light panel distinct from the window
behind it -- #eeeeee on #f5f5f5. Aligning the panel makes the two equal, which
is what mixer, picker and text-transformer have always done, and the edge
between window and panel goes away. That is the trade, and it was measured
rather than assumed: the adjacency map lists that pair as STACKED, with the
evidence, before the change was made.

The dark input goes the other way. It was the card colour, so a field sitting
on a card had no edge. At #1a1a1a it has one -- and matches the panel, which is
how the three apps that already use it draw a field. The border is what
separates a field from the panel there, and the guard asserts that border
differs from both.

IMAGE MODE IS DELIBERATELY UNTOUCHED. It was not part of the comparison this
ruling came from, and its panel is an alpha-prefixed #ED1A1A1A rather than a
flat colour.
"""
...
# A bare `pytest` also collects snapshots/test_import_snapshots.py, whose
# basename collides with the one in tests/ and halts collection.
SUITES = [
...
def edits(tree) -> None:
    ...
    b = _bounds(lines)
    _set(lines, b["LIGHT_THEME_COLORS"], "panel_bg", "'#eeeeee'", LIGHT_PANEL)
...
def checks(tree) -> None:
    ...
    if "'panel_bg': '#eeeeee'" in src:
        raise SystemExit("the old light panel survives")
