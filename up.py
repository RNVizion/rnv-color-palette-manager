...
Give every hex in rnv-color-palette-manager's palettes a constant, and collapse the strays.
...
32 palette entries in this repository were written as bare hex literals
-- 27 of them in the light palette. A literal cannot follow a register
...
In this repository #f0f0f0 is not in a palette at all. It is GREY_F0,
named on 2026-09-02 and painted directly by ui/image_upload_dialog.py --
which is why the fleet colour tree, which follows palette keys, reported
it as having no constant anywhere. It has one, here, and it collapses
with the other two. That dialog's preview ground moves 0.42 CIEDE2000.
It is the only pixel this script moves.
...
NEW = ['APP_SURFACE_LIGHT_3', 'GREY_E0', 'GREY_EE', 'GREY_CC', 'GREY_88', 'GREY_55', 'GREY_60']
DOCS = {'APP_SURFACE_LIGHT_3': ('#f5f5f5', 'engine/brand.py APP["surface-light-3"]. The light window and panel\nground -- what a dialog sits on in light mode.\n\nRNV-LIGHT-WIRING (2026-09-06): this value was written out as a literal\nin every palette that used it, so nothing could move it. Registered by\nrev 27 as the third rung of the light surface ladder; named here under\nthe register\'s key, the way APP_PANEL_HOVER and APP_HOVER_LIGHT are.\nEvery key that carries it is a surface, so it is not split.'), 'GREY_E0': ('#e0e0e0', 'grey(14) on the ramp, #e0e0e0. Static surfaces that share a hex with\nAPP_PRESSED_LIGHT without being a pressed state. See the split note\nthere. Named by its byte, like every other ramp step.'), 'GREY_EE': ('#eeeeee', 'grey(14) on the ramp, #eeeeee. Static surfaces that share a hex with\nAPP_HOVER_LIGHT without being a hover: a list header, a scroll ground.\nSame split rnv-text-transformer ruled for its diff headers.'), 'GREY_CC': ('#cccccc', 'grey(12) on the ramp, #cccccc. The light-mode border.'), 'GREY_88': ('#888888', 'grey(8) on the ramp, #888888. Muted text on dark, a scrollbar handle\nhover on light.'), 'GREY_55': ('#555555', 'grey(5) on the ramp, #555555. Disabled text and a checkbox edge on dark.'), 'GREY_60': ('#606060', 'grey(6) on the ramp, #606060. A scrollbar handle hover on dark.')}
ANCHOR = 'afford to lose. A boundary is not a plate.\n"""\n'
PROVENANCE_ANCHOR = '    "APP_PANEL_OVERLAY": "register-overlay",\n}\n'
...
SPLIT = {}
SPLIT_ABSENT = ['platform_btn_hover_bg', 'pressed_bg']
# old hex -> new hex
COLLAPSE = {'#fafafa': '#fbfbfb', '#f8f8f8': '#fbfbfb', '#f0f0f0': '#eeeeee'}
...
EXTRA_ALLOWED_PAIRS = (('SESSION_FALLBACK_COLOR', 'DEFAULT_SLOT_COLOR'),)
